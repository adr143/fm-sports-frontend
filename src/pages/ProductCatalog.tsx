...
const ProductCatalogPage = () => {
  ...
  const filteredProducts = products.filter(product => selectedCategory === 'All' || product.category.toLowerCase() === selectedCategory.toLowerCase()).filter(product => selectedSizes.length === 0 || selectedSizes.some(size => product.sizes.includes(size))).filter(product => selectedColors.length === 0 || selectedColors.some(color => product.colors?.map((c: string) => c.toLowerCase()).includes(color.toLowerCase()))).sort((a, b) => {
    switch (sortBy) {
